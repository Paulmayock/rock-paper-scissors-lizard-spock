...
function win(usersChoice, computersChoice) {
  ...
  if (userScore < 10) {
    result_p.innerHTML = `${LetterToWord(
      usersChoice
    )}<sub id ="subUser"></sub> beats ${LetterToWord(
      computersChoice
    )}<sub id ="subComp"></sub>. You win!`;
  ...
  setTimeout(function () {
    usersChoice_div.classList.remove("green-glow");
  }, 300);
}
...
function lose(usersChoice, computersChoice) {
  ...
  if (computerScore < 10) {
    result_p.innerHTML = `${LetterToWord(
      computersChoice
    )}<sub id ="subComp"></sub> beats ${LetterToWord(
      usersChoice
    )}<sub id ="subUser"></sub>. You lose`;
  ...
  setTimeout(function () {
    usersChoice_div.classList.remove("red-glow");
  }, 300);
}
...
function draw(usersChoice, computersChoice) {
  ...
  result_p.innerHTML = `${LetterToWord(
    usersChoice
  )}<sub id ="subUser"></sub> equals ${LetterToWord(
    computersChoice
  )}<sub id ="subComp"></sub>. It's a draw`;
  //The choice is highlighted by a yellow glow
  usersChoice_div.classList.add("yellow-glow");
  setTimeout(function () {
    usersChoice_div.classList.remove("yellow-glow");
  }, 300);
}
